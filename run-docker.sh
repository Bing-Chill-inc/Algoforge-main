...
check_requirements() {
    ...
    command -v docker >/dev/null 2>&1 || { echo >&2 "⚠️ Docker n'est pas installé. Veuillez l'installer depuis https://docker.com."; read -p "Appuyez sur Entrée pour continuer..."; return; }
    docker compose version >/dev/null 2>&1 || { echo >&2 "⚠️ Docker Compose n'est pas installé. Veuillez l'installer depuis https://docs.docker.com/compose/install/."; read -p "Appuyez sur Entrée pour continuer..."; return; }
}
...
rename_env_file() {
    if [ -f ".env" ]; then
        ...
        if [ "$response" = "o" ] || [ "$response" = "O" ]; then
            rm .env || { echo "⚠️ Échec de la suppression du fichier '.env'."; read -p "Appuyez sur Entrée pour continuer..."; return; }
        else
    ...
    cp template-docker.env .env || { echo "⚠️ Échec de la copie du fichier 'template-docker.env'."; read -p "Appuyez sur Entrée pour continuer..."; return; }
}
...
start_application() {
    ...
    docker compose up --wait || { echo "⚠️ Échec du démarrage de l'application avec docker compose."; read -p "Appuyez sur Entrée pour continuer..."; return; }
}
